...
public class Exercise1 {
        ...
        List<Person> personsEverWorkedInEpam =
                employees.stream()
                        .filter(e -> e.getJobHistory()
                                .stream()
                                .anyMatch(jh -> "EPAM".equals(jh.getEmployer())))
    ...
    public void findPersonsBeganCareerInEpam() {
        ...
        // TODO реализация, использовать Collectors.toList()
        List<Person> startedFromEpam = employees.stream().filter(p -> p.getJobHistory().get(0).getEmployer().equals("EPAM")).map(Employee::getPerson).collect(Collectors.toList());
