...
public class Exercise1 {
    ...
    private static BiFunction<Person, Person, Integer> createExtractorAgeOfPersonWithTheLongestFullName(Function<Person, String> fullNameExtractor) {
        return (person1, person2) -> {
            int person1FullNameLength = fullNameExtractor.apply(person1).length();
            int person2FullNameLength = fullNameExtractor.apply(person2).length();
            return person1FullNameLength > person2FullNameLength ? person1.getAge() : person2.getAge();
        };
     }
